...
const LoanRequests = () => {
  ...
  useEffect(() => {
    fetchUserId();
    fetchLoanRequestsByStatuses(['Preparing for Pickup','Ready for Pickup']);
  }, []);
  ...
  <tbody className="text-sm text-gray-700">
    {loanRequests.map((request) => (
      <tr key={request.loanId} className="border-t hover:bg-gray-50 transition duration-200">
        ...
        <td className="px-6 py-4">
        {request.status === 'Preparing for Pickup' && (
  <div className="flex space-x-2">
    <button
      onClick={() => handleStatusChange(request.loanId, 'Ready for Pickup')}
      className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 w-20 text-sm rounded-md transition duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
    >
      Ready
    </button>
    <button
      onClick={() => handleStatusChange(request.loanId, 'Cancelled')}
      className="bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 w-20 text-sm rounded-md transition duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50"
    >
      Cancel
    </button>
  </div>
