...
const RenewalRequests = () => {
  ...
  const fetchRenewalRequestsByStatuses = async (statuses) => {
    try {
      ...
      setError('Failed to fetch renewal requests. Please try again later.');
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchUserId();
    fetchRenewalRequestsByStatuses(['Pending Renewal','Renewed','Renewal Denied','Renewal Cancelled']);
  }, []);
  ...
  const handleStatusChange = async (loanId, status) => {
    try {
      ...
      if (response.status === 200) {
        toast.success('Renewal request status updated!');
        ...
        toast.error('Failed to update the loan renewal status.');
      }
    } catch (error) {
      console.error('Error updating renewal request status:', error);
      toast.error('Error updating renewal request status. Please try again later.');
    }
  };
  ...
  <tbody className="text-sm text-gray-700">
    {renewalRequests.map((request) => (
      <tr key={request.loanId} className="border-t hover:bg-gray-50 transition duration-200">
        ...
        <td className="px-6 py-4">
        {request.status === 'Pending Renewal' && (
            <button
